      ...
      <tbody>
        ...
        if ($_SESSION['grade'] == 1) {
          ...
          while ($item = $statement->fetch()) {
            ...
            echo '<td width=340>';
            echo '<a class="btn btn-secondary" href="edito_Admin?id='.$item['ID'].'"><span class="bi-eye"></span> Voir</a>';
            echo ' ';
          ...
          $db = Database::connect();
          $statement = $db->query("SELECT articles.ID ,`Titre`, `Resume`,`Image`FROM articles INNER JOIN utilisateur on articles.user_ID = utilisateur.ID WHERE utilisateur.ID = '$id' ORDER BY articles.ID DESC");
